...
struct SegTree
{
    ...
    int find(int x, int l, int node, int lx, int rx)
    {
        if (seg_data[node].mx < x || rx <= l)
            return -1;
        if (rx - lx == 1)
            return lx;
        int mid = (lx + rx) / 2;
        int res = find(x, l, 2 * node + 1, lx, mid);
        if (res == -1)
            res = find(x, l, 2 * node + 2, mid, rx);
        return res;
    }

    int find(int x, int l)
    {
        return find(x, l, 0, 0, tree_size);
    }
};
